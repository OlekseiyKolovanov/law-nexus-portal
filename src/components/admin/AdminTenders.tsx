          ...
          <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="has_form">Має форму заявки</Label>
              </div>
        ...
        {tenders.length === 0 ? (
          <Card>
            <CardContent className="text-center py-8">
              ...
              <p className="text-muted-foreground">Тендери поки що не створено</p>
            </CardContent>
          </Card>
        ) : (
          tenders.map((tender) => (
            <Card key={tender.id} className={!tender.is_active ? 'opacity-60' : ''}>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {tender.has_form && (
                        <Badge variant="outline">Має форму</Badge>
                      )}
                    </div>
                    <CardDescription>
                      Створено: {new Date(tender.created_at).toLocaleDateString('uk-UA')}
                    </CardDescription>
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      {tender.is_active ? 'Деактивувати' : 'Активувати'}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground whitespace-pre-wrap">{tender.content}</p>
              </CardContent>
            </Card>
          ))
        )}
